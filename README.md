...
Completed the implementations of the Sentence class and the MinesweeperAI class in minesweeper.py.
...
7. make_safe_move should return a move (i, j) that is known to be safe.
8. The move returned must be known to be safe, and not a move already made.
9. If no safe move can be guaranteed, the function should return None.
10. The function should not modify self.moves_made, self.mines, self.safes, or self.knowledge.
make_random_move should return a random move (i, j).
11. This function will be called if a safe move is not possible: if the AI doesn’t know where to move, it will choose to move randomly instead.
12. The move must not be a move that has already been made.
13. The move must not be a move that is known to be a mine.
